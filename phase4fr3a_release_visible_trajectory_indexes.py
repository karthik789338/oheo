...
OUTPUT_DIR = Path(
    "outputs/"
    "phase4fr3a_tier_c_v4_visible_trajectory_indexes"
)
...
def load_and_validate_structural_rows(
    ...
    with source_path.open(
        ...
        require(
            ...
            == set(SAFE_COLUMNS),
            (
                f"{source_path} does not contain "
                "exactly the approved structural schema.\n"
                f"Observed: {source_columns}\n"
                f"Allowed:  {SAFE_COLUMNS}"
            ),
        )
...
def main() -> None:
    ...
    summary = {
        "phase":
            (
                "4F-R3A Tier C v4 controlled "
                "structural-index release and "
        ...
        "privileged_scientific_fields_accessed":
            False,

        "clean_targets_accessed":
            False,

        "privileged_state_ids_accessed":
            False,
        ...
        "phase4fr3a_status":
            (
                ...
                "with_visible_structural_indexes"
            ),
    ...
    write_json(
        OUTPUT_DIR
        / "phase4fr3a_repaired_tuning_preflight_summary.json",
        summary,
    )
